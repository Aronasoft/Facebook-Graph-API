...
if(isset($_GET['page_id']) && ($_GET['page_id']!="")){
		if(isset($_GET['access_token']) && ($_GET['access_token']!="")){
			...
			for($i=0; $i<$count; $i++){
				...
				$catCount= count($totalLikes[$i]['category_list']);
				
				for($j=0; $catCount>$j; $j++){
					
					$newcatId['name'][] = $totalLikes[$i]['category_list'][$j]['id'].'+'.$totalLikes[$i]['category_list'][$j]['name'];
					
					$catSql = "INSERT IGNORE INTO fb_page_category_list (cat_id, name) VALUE ('".$totalLikes[$i]['category_list'][$j]['id']."', '".$totalLikes[$i]['category_list'][$j]['name']."')";
					$query1 = mysqli_query($conn, $catSql);
					
				}
				$catIdName = implode('|', $newcatId['name']);
				...
				if(!empty($totalLikes[$i]['location'])){
				   ...
				   $sql = "INSERT INTO fb_page_data_tbl (parent_page_id, liked_page_id, page_like_count, liked_page_name, liked_page_username, liked_page_link, liked_page_category_list, liked_page_categoryPlist, liked_page_category_id, liked_page_location, liked_page_profile_image ) VALUES ('".$page_id."', '".$totalLikes[$i]['id']."', '".$pageLikeCount."','".mysqli_real_escape_string($conn, $totalLikes[$i]['name'])."', '".mysqli_real_escape_string($conn, $totalLikes[$i]['username'])."', '".mysqli_real_escape_string($conn, $totalLikes[$i]['link'])."', '".mysqli_real_escape_string($conn, @$totalLikes[$i]['category_list_serial'])."', '".mysqli_real_escape_string($conn, @$imp)."', '".@mysqli_real_escape_string($conn, $catIdName)."', '".@mysqli_real_escape_string($conn, $totalLikes[$i]['location_serial'])."', '".mysqli_real_escape_string($conn, $totalLikes[$i]['picture'])."')";
